...
async function lookupBankCode(bankNameRaw) {
    try {
        ...
        console.error("âš ï¸ Korapay Bank Code Error (non-fatal):", error?.response?.data || error.message);
        return null;
    }
}
...
async function createKorapayVirtualAccount(customerPhone, amount, productName) {
    try {
        ...
        console.error("âš ï¸ Korapay Virtual Account Error:", error?.response?.data || error.message);
...
async function safeSendMessage(sock, jid, content, options = {}) {
    ...
    const sendPromise = sendQueue[queueKey].then(async () => {
        try {
            ...
            console.error("âŒ safeSendMessage Error:", err.message);
            return null;
        }
    });
...
function getStepPrompt(step, session = {}) {
    ...
    switch (step) {
        case 1: 
            return "ðŸ“ *Step 1/8:* What is your Business / Store Name? âœ¨";
        case 2: 
            return `Store Name saved: *${storeName}* âœ…\n\nðŸ·ï¸ *Step 2/8:* What category is your business?\n\n` +
                   `Reply with one of the numbers below:\n` +
                   `1ï¸âƒ£ *Retail & Products* (Fashion, Electronics, Food, General Goods)\n` +
                   `2ï¸âƒ£ *Custom / Bespoke* (Custom Cakes, Tailoring, Handcrafted, Wholesale)\n` +
                   `3ï¸âƒ£ *Services & Transport* (Campus Shuttle, Taxi, Logistics, Barber, Consultations)`;
        case 3: 
            return "ðŸ“– *Step 3/8:* Give a short description of what your business does. ðŸ’¡";
        case 4:
            if (session.businessType === 'CUSTOM') {
                return "ðŸ¤ *Step 4/8:* Do you allow AI price negotiations with buyers?\n\n" +
                ...
                return "ðŸ’³ *Step 4/8:* How should customers pay for your service/transport?\n\n" +
                       "Reply with:\n" +
                       "1ï¸âƒ£ *PAY_ON_BOARD* (Students/clients pay cash/transfer upon boarding/service)\n" +
                       "2ï¸âƒ£ *UPFRONT* (Payment required before booking confirmation)\n" +
                       "3ï¸âƒ£ *FLEXIBLE* (Both allowed)";
            } else {
                return "ðŸ“± *Step 5/8:* Enter your **WhatsApp Phone Number** for linking your AI (e.g., 2348027986674). ðŸ“ž";
            }
        case 5: 
            return "ðŸ“± *Step 5/8:* Enter your **WhatsApp Phone Number** for linking your AI (e.g., 2348027986674). ðŸ“ž";
        case 6: 
            return "ðŸ’³ *Step 6/8:* Provide your Bank Name and Account Number (e.g. *Opay - 8148698365*).\n\n" +
                   "_(Note: If your business doesn't collect online payments, reply *SKIP*)._ ðŸ¦";
        case 7: 
            return "ðŸšš *Step 7/8:* How do you handle delivery/pickup? (e.g. *Same day in Campus*, *Pickup at Garage*, *Nationwide GIGM*). ðŸ“¦";
        case 8: 
            if (session.businessType === 'SERVICE_TRANSPORT') {
                return "ðŸš• *Step 8/8:* List your routes or services with prices!\n\n" +
                       "Reply with text like:\n`Main Gate to Hostels - 200`\n`Campus Shuttle Daily Pass - 1000`\n\n" +
                       "Reply *DONE* when finished listing! âœ¨";
            } else {
                return "ðŸ“¸ *Step 8/8:* Add your products or services!\n\n" +
                       "You can send product photos with captions (e.g. `Vintage Shirt - 12000`), or text only.\n\n" +
                       "When done, reply with *DONE*. âœ¨";
            }
        default: return "";
    }
}
...
async function spawnVendorAgent(realPhone, storeName, requestNewCode = false) {
    ...
    if (!vendorSock.authState.creds.registered && requestNewCode) {
        ...
        try {
            ...
            console.log(`\nðŸ”‘ VALID VENDOR PAIRING CODE FOR ${storeName} (${cleanPhone}): ${pairingCode}\n`);
    ...
    vendorSock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            console.log(`ðŸš€ Vendor Agent LIVE for ${storeName} (${cleanPhone})!`);
            if (typeof notifyVendorClients === 'function') {
            ...
            try {
                ...
                if (vendorData && !vendorData.docsSent) {
                    const docsMessage = `ðŸŽ‰ *CONGRATULATIONS! YOUR NAXR AI AGENT IS NOW LIVE!* ðŸš€\n\n` +
                        ...
                        `ðŸ“– *QUICK OPERATIONAL GUIDE*\n` +
                        `â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n` +
                        `1ï¸âƒ£ *Automated Catalog:* Customers can ask for your catalog, and the AI will auto-send your product pictures and prices.\n` +
                        `2ï¸âƒ£ *Virtual Accounts (Anti-Fraud):* Instead of links, Naxr generates a direct **Virtual Bank Account** for every transaction. Fake screenshots won't work anymoreâ€”the AI verifies payments instantly via Flutterwave and wires the money to you!\n\n` +
                        `ðŸ› ï¸ *MANAGE YOUR STORE DIRECTLY HERE*\n` +
                        `Message yourself (this chat) with these commands:\n` +
                        `â€¢ *stats* - View your total sales.\n` +
                        `â€¢ *analytics* - Detailed breakdown of orders & revenue.\n` +
                        `â€¢ *products* - See your current list of items.\n` +
                        `â€¢ *ai off* / *ai on* - Toggle the AI agent.\n` +
                        `â€¢ *edit description [new text]* - Update your business info.\n` +
                        `â€¢ *delete product [name]* - Remove an item.\n` +
                        `â€¢ *confirm test* - Manually confirm a test payment (test mode only).\n` +
                        `â€¢ *Add new item:* Simply send a picture of the product to this chat and write the price and name in the caption!\n\n` +
                        `âœ¨ *You are now ready to scale your business on autopilot!* ðŸ¥‚`;
                ...
                if (isVendorSelfChat && (isVendorCommand || isProductAddImage)) {
                    if (vendorData) {
                        ...
                        if (daysActive > 7 && !vendorData.isPro) {
                            const trialExpiredMessage = `âš ï¸ *TRIAL EXPIRED:* Your 7-day Naxr AI trial has ended. Your bot is currently paused.\n\n` +
                                `To renew your subscription and keep automated sales running, please make payment to:\n\n` +
                                `ðŸ¦ *Bank:* Kuda Microfinance Bank\n` +
                                `ðŸ”¢ *Account Number:* 3003853004\n` +
                                `ðŸ‘¤ *Account Name:* KUKA TECHNOLOGY AND INNOVATION LIMITED\n\n` +
                                `ðŸ‘‰ *After payment, simply send the transaction receipt screenshot to this chat.* Our system will verify the receipt and automatically reactivate your service! ðŸš€`;
                            await safeSendMessage(vendorSock, remoteJid, { text: trialExpiredMessage });
                            continue;
                        }
                    }

                    // â”€â”€â”€ VENDOR SELF-CHAT ADMIN CONTROLS â”€â”€â”€
                    if (lowerText === 'ai off') {
                        await Vendor.findOneAndUpdate({ phoneNumber: cleanVendorPhone }, { aiActive: false });
                        await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ›‘ *AI Agent is now OFF.*\n\nCustomers will no longer receive automated replies until you turn it back on.` });
                        continue;
                    }
                    if (lowerText === 'ai on') {
                        await Vendor.findOneAndUpdate({ phoneNumber: cleanVendorPhone }, { aiActive: true });
                        await safeSendMessage(vendorSock, remoteJid, { text: `âœ… *AI Agent is now ON.*\n\nYour store is live and ready to take orders!` });
                        continue;
                    }
                    ...
                    if (lowerText === 'stats' || lowerText === 'sales' || lowerText === 'dashboard') {
                        ...
                        let trialText = vendorData?.isPro ? "âœ… Pro Plan Active" : `â³ Free Trial: ${Math.max(7 - Math.floor((Date.now() - new Date(vendorData.createdAt).getTime()) / (1000 * 60 * 60 * 24)), 0)} days left`;
                        let aiStatus = vendorData?.aiActive !== false ? "ðŸŸ¢ AI ON" : "ðŸ”´ AI OFF";

                        await safeSendMessage(vendorSock, remoteJid, {
                            text: `ðŸ“Š *${storeName} Admin Dashboard*\n\n` +
                                `ðŸ›ï¸ Active Products: ${productsCount}\n` +
                                `âœ… Confirmed Paid Sales: ${salesCount}\n` +
                                `â³ Pending Orders: ${pendingCount}\n` +
                                `ðŸ’° Total Revenue: â‚¦${revenue.toLocaleString()}\n\n` +
                                `${trialText}\n${aiStatus}`
                        });
                        continue;
                    }
                    ...
                    if (lowerText === 'analytics') {
                        ...
                        const recentPaid = paidOrders.length > 0
                            ? paidOrders.map(o => `â€¢ ${o.productName} â€” â‚¦${o.amount.toLocaleString()} (+${o.customerPhone})`).join('\n')
                            : "No paid orders yet.";

                        const recentPending = pendingOrders.length > 0
                            ? pendingOrders.map(o => `â€¢ ${o.productName} â€” â‚¦${o.amount.toLocaleString()} (Ref: ${o.virtualAccountNumber})`).join('\n')
                            : "No pending orders.";

                        await safeSendMessage(vendorSock, remoteJid, {
                            text: `ðŸ“ˆ *Analytics for ${storeName}*\n\n` +
                    ...
                    if (lowerText === 'products' || lowerText === 'catalog') {
                        ...
                        if (activeProducts.length > 0) {
                            await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ“¦ *Product Catalog for ${storeName}:*` });
                            for (const p of activeProducts) {
                                if (p.imageUrl) {
                                    try {
                                        await safeSendMessage(vendorSock, remoteJid, {
                                            image: { url: p.imageUrl },
                                            caption: `ðŸ›ï¸ *${p.name}*\nðŸ’° Price: â‚¦${p.price.toLocaleString()}`
                                        });
                                    } catch (e) {
                                        await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ›ï¸ *${p.name}*\nðŸ’° Price: â‚¦${p.price.toLocaleString()}` });
                                    }
                                } else {
                                    await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ›ï¸ *${p.name}*\nðŸ’° Price: â‚¦${p.price.toLocaleString()}` });
                                }
                            ...
                            await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ“¦ No products added yet.` });
                        }
                        continue;
                    }
                    ...
                    if (lowerText.startsWith('edit description ')) {
                        ...
                        await safeSendMessage(vendorSock, remoteJid, { text: `âœ… Business description updated.` });
                        continue;
                    }
                    ...
                    if (lowerText.startsWith('delete product ')) {
                        ...
                        if (res.deletedCount > 0) await safeSendMessage(vendorSock, remoteJid, { text: `âœ… Product "${prodName}" deleted.` });
                        else await safeSendMessage(vendorSock, remoteJid, { text: `âš ï¸ Product not found.` });
                        continue;
                    }
                    ...
                    if (lowerText === 'confirm test' || lowerText === 'test confirm') {
                        ...
                        if (!pendingOrder) {
                            await safeSendMessage(vendorSock, remoteJid, { text: `âš ï¸ No pending orders found.` });
                            continue;
                        }
                    ...
                    if (isImage) {
                        ...
                        if (match) {
                            ...
                            const name = textMessage.replace(match[0], '').replace(/[#â‚¦$-]/g, '').trim() || "Unnamed Item";
                            await safeSendMessage(vendorSock, remoteJid, { react: { text: "â³", key: msg.key } });
                            const buffer = await downloadMediaMessage(msg, 'buffer', {});
                            const imageUrl = await uploadToCloudinary(buffer);
                            await Product.create({ vendorPhone: cleanVendorPhone, name, price, imageUrl });
                            await safeSendMessage(vendorSock, remoteJid, { text: `âœ… *New Product Added!*\n\n${name} - â‚¦${price.toLocaleString()}` });
                        } else {
...
Reply in JSON format only: {"isSubscriptionReceipt": true/false, "isSuspicious": true/false, "confidence": "high/medium/low", "reason": "Explain details."}` 
                                ...
                                if (analysis.isSubscriptionReceipt && !analysis.isSuspicious) {
                                    ...
                                    await safeSendMessage(vendorSock, remoteJid, { react: { text: "âœ…", key: msg.key } });
                                    await safeSendMessage(vendorSock, remoteJid, { 
                                        text: `ðŸŽ‰ *PAYMENT VERIFIED SUCCESSFULLY!*\n\nThank you! Your Naxr AI Pro Subscription has been activated. Your store is now active and automating sales again! ðŸš€` 
                                    });

                                    // Notify Admin of new activation
                                    if (globalSock) {
                                        await safeSendMessage(globalSock, `${ADMIN_PHONE}@s.whatsapp.net`, {
                                            text: `ðŸ‘‘ *NEW SUBSCRIPTION ACTIVATED!*\n\nVendor: ${storeName} (${cleanVendorPhone})\nReceipt Status: Verified by AI (${analysis.confidence} confidence).\nReason: ${analysis.reason}`
                                        });
                                        await safeSendMessage(globalSock, `${ADMIN_PHONE}@s.whatsapp.net`, {
                                            image: buffer,
                                            caption: `ðŸ“„ Subscription Receipt for ${storeName} (${cleanVendorPhone})`
                                        });
                                    }
                                } else {
                                    await safeSendMessage(vendorSock, remoteJid, { react: { text: "âŒ", key: msg.key } });
                                    await safeSendMessage(vendorSock, remoteJid, { 
                                        text: `âš ï¸ *RECEIPT VERIFICATION FAILED:*\n\nReason: ${analysis.reason}\n\nIf you believe this is an error, please contact support for manual activation.` 
                                    });
                                    
                                    // Alert admin of failed receipt upload
                                    if (globalSock) {
                                        await safeSendMessage(globalSock, `${ADMIN_PHONE}@s.whatsapp.net`, {
                                            text: `âš ï¸ *FAILED/SUSPICIOUS SUBSCRIPTION RECEIPT DETECTED!*\n\nVendor: ${storeName} (${cleanVendorPhone})\nAI Review: ${analysis.reason}`
                                        });
                                        await safeSendMessage(globalSock, `${ADMIN_PHONE}@s.whatsapp.net`, {
                                            image: buffer,
                                            caption: `ðŸ“„ Failed Subscription Receipt for ${storeName} (${cleanVendorPhone})`
                                        });
                                    }
                                }
                            } catch (err) {
                                await safeSendMessage(vendorSock, remoteJid, { text: `âš ï¸ Could not verify receipt at this time: ${err.message}` });
                            }
                        }
                        continue;
                    }
                    ...
                    if (lowerText === 'help') {
                        await safeSendMessage(vendorSock, remoteJid, {
                            text: `ðŸ’¡ *Naxr Vendor Commands:*\n` +
                                `â€¢ *stats* â€” Quick sales summary\n` +
                                `â€¢ *analytics* â€” Detailed breakdown\n` +
                                `â€¢ *products* â€” View catalog\n` +
                                `â€¢ *ai on* / *ai off* â€” Toggle AI\n` +
                                `â€¢ *edit description [text]*\n` +
                                `â€¢ *delete product [name]*\n` +
                                `â€¢ *confirm test* â€” Confirm pending order\n` +
                                `â€¢ Send image + price to add product`
                        });
                        continue;
                    }
                ...
                // â”€â”€â”€ CUSTOMER-FACING AI â”€â”€â”€
...
2. Does the amount shown on the receipt match â‚¦${pendingOrder.amount} exactly?
...
Reply in JSON format only: {"isReceipt": true/false, "isSuspicious": true/false, "confidence": "high/medium/low", "reason": "Explain details of what you found."}` 
                            ...
                            if (analysis.isReceipt && !analysis.isSuspicious) {
                                ...
                                receiptVerificationInfo = `ðŸš¨ SUSPICIOUS/FLAGGED: ${analysis.reason}`;
                                await safeSendMessage(vendorSock, remoteJid, { react: { text: "âš ï¸", key: msg.key } });
                            }
                        } catch (err) {
                    ...
                    const receiptText = `ðŸ§¾ *NAXR TRANSACTION RECEIPT*\n` +
                        `â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n` +
                    ...
                    const vendorAlertMessage = isVisionFlaggedSuspicious 
                        ? `ðŸš¨ *POSSIBLE FAKE RECEIPT DETECTED!*\n\n` +
                          `Customer +${cleanRemoteJidNumber} sent a suspicious receipt for *${pendingOrder.productName}* (â‚¦${pendingOrder.amount.toLocaleString()}).\n\n` +
                          `âš ï¸ *AI Fraud Check:* ${receiptVerificationInfo}\n\n` +
                          `ðŸ‘‰ *Do NOT deliver* until you verify this transfer inside your bank app. If valid, reply *"confirm test"* to confirm payment.`
                        : `ðŸ“© *CUSTOMER SUBMITTED PAYMENT RECEIPT!*\n\n` +
                    });
                }
                ...
                if (isCatalogRequest) {
                    if (activeProducts.length > 0) {
                        await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ“¦ *Here is our current catalog for ${storeName}:*` });
                        for (const p of activeProducts) {
                            if (p.imageUrl) {
                                try {
                                    await safeSendMessage(vendorSock, remoteJid, {
                                        image: { url: p.imageUrl },
                                        caption: `ðŸ›ï¸ *${p.name}*\nðŸ’° *Price:* â‚¦${p.price.toLocaleString()}\n\nðŸ‘‰ Reply *"I want to buy ${p.name}"* to place an order!`
                                    });
                                } catch (e) {
                                    await safeSendMessage(vendorSock, remoteJid, {
                                        text: `ðŸ›ï¸ *${p.name}*\nðŸ’° *Price:* â‚¦${p.price.toLocaleString()}\n\nðŸ‘‰ Reply *"I want to buy ${p.name}"* to place an order!`
                        ...
                        await delay(1000);
                        await safeSendMessage(vendorSock, remoteJid, { text: `âœ¨ Reply with the name of any item you'd like to buy!` });
                    } else {
                        await safeSendMessage(vendorSock, remoteJid, { text: `ðŸ“¦ We are currently updating our catalog.` });
                    }
                    continue;
                }
                ...
                const catalog = activeProducts.map(p => {
                    const minStr = (p.isNegotiable && p.minPrice > 0) ? ` (Listed: â‚¦${p.price}, Minimum allowed price: â‚¦${p.minPrice})` : ` (Price: â‚¦${p.price})`;
                ...
                if (reply.startsWith('{') && reply.endsWith('}')) {
                    try {
                        ...
                        if (isPayOnBoard) {
                            ...
                            await safeSendMessage(vendorSock, remoteJid, {
                                text: `ðŸšŒ *Booking Confirmed: ${data.productName}*\n\n` +
                                    `ðŸ’° *Fare / Price:* â‚¦${data.price.toLocaleString()}\n` +
                                    `Ref Code: *${orderRefNumber}*\n` +
                                    `ðŸ’³ *Payment Policy:* Pay cash/transfer upon boarding or service delivery. âœ¨\n\n` +
                                    `Thank you for booking with *${storeName}*! See you soon! ðŸ™Œ`
                            });
                            // Notify vendor
                            await safeSendMessage(vendorSock, `${cleanVendorPhone}@s.whatsapp.net`, {
                                text: `ðŸ”” *NEW SERVICE BOOKING!*\n\nService/Route: ${data.productName}\nFare: â‚¦${data.price.toLocaleString()}\nCustomer: +${cleanRemoteJidNumber}\nPayment Mode: Pay on Boarding/Delivery`
                            });
                        } else {
                            try {
                                await safeSendMessage(vendorSock, remoteJid, { text: `â³ *Generating your secure payment account details...*` });
                                ...
                                await safeSendMessage(vendorSock, remoteJid, {
                                    text: `ðŸ›ï¸ *Order Initiated: ${data.productName}*\n\n` +
                                        `ðŸ’° *Amount Due:* â‚¦${data.price.toLocaleString()}\n\n` +
                                        `ðŸ¦ *Payment Account Details (Kora Bank Transfer):*\n` +
                                        `â€¢ Bank: *${vAcc.bankName}*\n` +
                                        `â€¢ Account Number: *${vAcc.accountNumber}*\n` +
                                        `â€¢ Account Name: *${vAcc.accountName}*\n\n` +
                                        `ðŸ‘‰ Please transfer â‚¦${data.price.toLocaleString()} to the account above. Your payment will be verified automatically in 1-2 minutes! âœ¨`
                                });
                            } catch (err) {
                                console.error("âŒ Korapay account generation error, falling back to static:", err.message);
                                const orderRefNumber = `NX-${Date.now().toString().slice(-6)}`;
                                ...
                                await safeSendMessage(vendorSock, remoteJid, {
                                    text: `ðŸ›ï¸ *Order Initiated: ${data.productName}*\n\n` +
                                        `ðŸ’° *Amount Due:* â‚¦${data.price.toLocaleString()}\n\n` +
                                        `ðŸ¦ *Payment Account Details:*\n` +
                                        `Bank & Account: *${vendorBank}* (Manual Transfer)\n` +
                                        `Order Ref: *${orderRefNumber}*\n\n` +
                                        `ðŸ‘‰ Please make payment to the account above and reply by sending *PAID* or sharing your receipt! âœ¨\n` +
                                        `*(Note: The vendor will verify your transfer and confirm your order).*`
                                });
                            }
                        }
                    } catch (e) {
                        console.error("âŒ Checkout Processing Error:", e.message, e.stack);
                        await safeSendMessage(vendorSock, remoteJid, { text: "âš ï¸ Could not initiate order details at this time." });
                    }
                } else {
                    ...
                    if (matchedProduct && matchedProduct.imageUrl) {
                        try {
                            await safeSendMessage(vendorSock, remoteJid, {
                                ...
                                caption: `ðŸ›ï¸ *${matchedProduct.name}*\nðŸ’° *Price:* â‚¦${matchedProduct.price.toLocaleString()}`
                            });
                        } catch (e) { }
                    }
                }
            } catch (error) {
                console.error(`âŒ Message Processing Error for ${msg?.key?.remoteJid}:`, error);
            }
        }
    });
...
async function startNaxrMasterAgent(isReconnect = false) {
    ...
    if (!sock.authState.creds.registered && !isReconnect) {
        ...
        try {
            ...
            console.log(`\n======================================`);
            console.log(`ðŸ”‘ NAXR MASTER PAIRING CODE: ${code}`);
            console.log(`======================================\n`);
        } catch (err) { }
    }
    ...
    sock.ev.on('connection.update', (update) => {
        const { connection, lastDisconnect } = update;
        if (connection === 'open') console.log("ðŸš€ NAXR MASTER ONBOARDING AGENT IS LIVE! ðŸ‡³ðŸ‡¬");
        if (connection === 'close') {
            const statusCode = lastDisconnect?.error?.output?.statusCode;
            ...
            try {
                ...
                if (isAdminMessage) {
                    if (lowerText === 'admin stats' || lowerText === 'stats') {
                        ...
                        await safeSendMessage(sock, remoteJid, { text: `ðŸ‘‘ *Naxr Super Admin*\n\nðŸ‘¥ Total Vendors: ${totalVendors}\nðŸ›ï¸ Total Products Listed: ${totalProducts}\nâœ… Total Paid Orders: ${totalOrders}` });
                        continue;
                    }
                    if (lowerText === 'admin vendors' || lowerText === 'vendors') {
                        const vendors = await Vendor.find({});
                        const vList = vendors.map(v => `â€¢ ${v.storeName} (${v.phoneNumber})`).join("\n");
                        await safeSendMessage(sock, remoteJid, { text: `ðŸ‘‘ *Registered Vendors:*\n\n${vList || "No vendors yet."}` });
                        continue;
                    }
                    if (lowerText.startsWith('delete vendor ')) {
                        ...
                        await safeSendMessage(sock, remoteJid, { text: `âœ… Vendor ${targetPhone} completely purged from system.` });
                        continue;
                    }
                    if (lowerText.startsWith('activate vendor ')) {
                        ...
                        if (v) {
                            await safeSendMessage(sock, remoteJid, { text: `âœ… Vendor *${v.storeName}* (${targetPhone}) is now set to **Pro/Active**!` });
                        } else {
                            await safeSendMessage(sock, remoteJid, { text: `âš ï¸ Vendor with phone number ${targetPhone} not found.` });
                        }
                        continue;
                    }
                    if (lowerText.startsWith('cancel vendor ')) {
                        ...
                        if (v) {
                            await safeSendMessage(sock, remoteJid, { text: `ðŸ›‘ Vendor *${v.storeName}* (${targetPhone}) subscription canceled. Reverted to standard trial tier.` });
                        } else {
                            await safeSendMessage(sock, remoteJid, { text: `âš ï¸ Vendor with phone number ${targetPhone} not found.` });
                        }
                        continue;
                ...
                if (cleanText === 'reset' || cleanText === 'restart') {
                    ...
                    await sock.sendMessage(remoteJid, { text: `ðŸ”„ *Progress Reset!*\n\nOld store data cleared. Let's start over.\n\n` + getStepPrompt(1) });
                    continue;
                }
                ...
                if (existingVendor) {
                    if (cleanText === 'link' || cleanText === 'code' || cleanText === 'relink' || cleanText === 'new code') {
                        ...
                        await sock.sendMessage(remoteJid, { text: `ðŸ”„ Generating a fresh pairing code for *${existingVendor.storeName}*...` });
                        ...
                        if (newCode && newCode !== "ALREADY_ACTIVE" && newCode !== "ERROR") {
                            await sock.sendMessage(remoteJid, {
                                text: `ðŸ”‘ *YOUR VENDOR AI PAIRING CODE:* \`${newCode}\`\n\nðŸ›¡ï¸ *Note:* Because Naxr is an automated AI, WhatsApp may show a security warning asking if you know who is linking the device. Tap *"Continue"* to authorize your bot.\n\n1. Go to WhatsApp Settings > Linked Devices > Link a Device.\n2. Tap Link with phone number instead.\n3. Enter the code above.`
                            });
                        } else if (newCode === "ALREADY_ACTIVE") await sock.sendMessage(remoteJid, { text: `âœ… Your AI is already connected and active!` });
                        else await sock.sendMessage(remoteJid, { text: `âš ï¸ Network delay. Please reply with *LINK* again in 5 seconds.` });
                    } else if (cleanText === 'done') {
                        const targetPhone = cleanPhoneNumber(existingVendor.phoneNumber);
                        const vSock = vendorSockets[targetPhone];
                        if (vSock && vSock.authState.creds.registered) await sock.sendMessage(remoteJid, { text: `âœ… Your store *${existingVendor.storeName}* is already active!` });
                        else await sock.sendMessage(remoteJid, { text: `âš ï¸ Your store is registered but not connected right now. Reply with *LINK* to get a new code!` });
                    } else if (isRegTrigger) await sock.sendMessage(remoteJid, { text: `âš ï¸ Store *${existingVendor.storeName}* is registered. Reply with *LINK* to get a new pairing code!` });
                    continue;
                }
                ...
                if (isRegTrigger) {
                    if (!session) {
                        ...
                        await sock.sendMessage(remoteJid, { text: `ðŸ‘‹ Welcome to *Naxr*!\n\n` + getStepPrompt(1) });
                    } else await sock.sendMessage(remoteJid, { text: `ðŸ”„ *Resuming Onboarding*\n\n` + getStepPrompt(session.step, session.storeName) });
                    continue;
                }

                if (!session) {
                    await sock.sendMessage(remoteJid, { text: `ðŸ‘‹ Hi there! Are you looking to automate sales & booking for your business with Naxr AI?\n\nðŸ‘‰ Reply *"REGISTER"* or *"YES"* to set up your AI store agent in 2 minutes! âœ¨` });
                    continue;
                }
                session.updatedAt = new Date();

                if (!textMessage && !isImage) {
                    await sock.sendMessage(remoteJid, { text: `âš ï¸ *Invalid Input:* Please reply with text or an image.` });
                    continue;
                }

                if (session.step === 1) {
                    if (textMessage.length < 2) { await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Name must be at least 2 characters.\n\n` + getStepPrompt(1, session) }); continue; }
                ...
                if (session.step === 2) {
                    ...
                    session.step = 3; await session.save(); await sock.sendMessage(remoteJid, { text: `Category set to *${session.category}*! ðŸ‘\n\n` + getStepPrompt(3, session) }); continue;
                }

                if (session.step === 3) {
                    if (textMessage.length < 5) { await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Description too short.\n\n` + getStepPrompt(3, session) }); continue; }
                    session.description = textMessage; session.step = 4; await session.save(); await sock.sendMessage(remoteJid, { text: `Description saved! ðŸŽ¯\n\n` + getStepPrompt(4, session) }); continue;
                }
                ...
                if (session.step === 4) {
                    if (session.businessType === 'CUSTOM') {
                        ...
                        session.step = 5; await session.save(); await sock.sendMessage(remoteJid, { text: `Negotiation settings saved! ðŸ¤\n\n` + getStepPrompt(5, session) }); continue;
                    } else if (session.businessType === 'SERVICE_TRANSPORT') {
                        ...
                        session.step = 5; await session.save(); await sock.sendMessage(remoteJid, { text: `Payment policy saved: *${session.paymentPolicy}*! ðŸ’³\n\n` + getStepPrompt(5, session) }); continue;
                    } else {
                        const clean = cleanPhoneNumber(textMessage);
                        if (clean.length < 10) { await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Invalid phone number format.` }); continue; }
                        session.vendorRealPhone = clean; session.step = 6; await session.save(); await sock.sendMessage(remoteJid, { text: `Phone number *${clean}* saved! ðŸ“ž\n\n` + getStepPrompt(6, session) }); continue;
                    }
                }
                ...
                if (session.step === 6) {
                    if (cleanText === 'skip' || cleanText === 'no' || cleanText === 'none') {
                        ...
                        session.step = 7; await session.save(); await sock.sendMessage(remoteJid, { text: `Bank details skipped! â©\n\n` + getStepPrompt(7, session) }); continue;
                    }
                    const parts = textMessage.split('-');
                    if (parts.length < 2) { await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Use format "Bank Name - Account Number" or reply *SKIP*.\n\n` + getStepPrompt(6, session) }); continue; }

                    const bankName = parts[0].trim();
                    const accNo = parts[1].replace(/[^0-9]/g, '');
                    if (accNo.length < 10) { await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Account number must be 10 digits or reply *SKIP*.\n\n` + getStepPrompt(6, session) }); continue; }

                    await sock.sendMessage(remoteJid, { text: `â³ Saving your bank details...` });
                    const bankCode = await lookupBankCode(bankName);

                    session.bankDetails = `${bankName} - ${accNo}`;
                    session.subaccountCode = bankCode || null;
                    session.step = 7; await session.save(); await sock.sendMessage(remoteJid, { text: `Bank details saved! ðŸ”’\n\n` + getStepPrompt(7, session) }); continue;
                }

                if (session.step === 7) {
                    if (textMessage.length < 3) { await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Please provide more delivery/pickup details.\n\n` + getStepPrompt(7, session) }); continue; }
                    session.deliveryInfo = textMessage; session.step = 8; await session.save(); await sock.sendMessage(remoteJid, { text: `Delivery/Pickup info saved! ðŸ“\n\n` + getStepPrompt(8, session) }); continue;
                }

                if (session.step === 8) {
                    if (cleanText === 'done' || cleanText === 'finish') {
                        const targetPhone = cleanPhoneNumber(session.vendorRealPhone || remoteJid);
                        await sock.sendMessage(remoteJid, { text: `â³ *Finalizing setup... Generating your AI pairing code. Please wait a few seconds...*` });
                        ...
                        if (pairingCode && pairingCode !== "ALREADY_ACTIVE" && pairingCode !== "ERROR") {
                            await sock.sendMessage(remoteJid, { text: `ðŸŽ‰ *SETUP COMPLETED SUCCESSFULLY!* ðŸš€\n\nYour store *${session.storeName}* is active!\n\nðŸ”‘ *YOUR VENDOR AI PAIRING CODE:* \`${pairingCode}\`\n\nðŸ›¡ï¸ *Note:* Because Naxr is an automated AI, WhatsApp may show a security warning asking if you know who is linking the device. Tap *"Continue"* to authorize your bot.\n\n*How to link your AI:*\n1. Open WhatsApp Settings > Linked Devices > Link a Device > Link with phone number instead.\n2. Enter code on phone number *+${targetPhone}*! âœ¨` });
                        } else if (pairingCode === "ALREADY_ACTIVE") {
                            await sock.sendMessage(remoteJid, { text: `ðŸŽ‰ *SETUP COMPLETED!* Your AI is already connected and active! âœ¨` });
                        } else {
                            await sock.sendMessage(remoteJid, { text: `ðŸŽ‰ *SETUP COMPLETED!* ðŸš€\n\nYour store is fully saved!\n\nâš ï¸ *Meta's network experienced a slight delay so your pairing code couldn't be instantly generated.*\n\nðŸ‘‰ *Please reply with LINK right now to generate your code!*` });
                        }
                        continue;
                    }
                    ...
                    if (isImage) {
                        ...
                        if (match) {
                            ...
                            name = textMessage.replace(match[0], '').replace(/[#â‚¦$-]/g, '').trim() || "Unnamed Item";
                        }

                        if (!name || price === 0) {
                            session.pendingProductImage = imageUrl; await session.save();
                            await sock.sendMessage(remoteJid, { react: { text: "â“", key: msg.key } });
                            await sock.sendMessage(remoteJid, { text: `ðŸ“¸ *Photo received!* Reply with Product Name and Price (e.g., \`Vintage Shirt - 12000\`).` });
                        } else {
                            ...
                            await sock.sendMessage(remoteJid, { text: `âœ… *Item Saved:* ${name} (â‚¦${price.toLocaleString()})\n\nSend another, or reply *DONE* to finish! âœ¨` });
                        }
                        continue;
                    }
                    ...
                    if (session.pendingProductImage && textMessage) {
                        ...
                        if (price > 0) {
                            ...
                            await sock.sendMessage(remoteJid, { text: `âœ… *Item Saved:* ${name} (â‚¦${price.toLocaleString()})\n\nSend another photo, or reply *DONE* to finish! âœ¨` });
                        } else {
                            await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Please include the price numbers (e.g., \`Shirt - 12000\`).` });
                        }
                        continue;
                    }

                    if (!isImage && cleanText !== 'done' && !session.pendingProductImage) {
                        await sock.sendMessage(remoteJid, { text: `âš ï¸ *Feedback:* Please send a product photo, or reply *DONE* if you are finished adding products.` });
                    }
                }
            } catch (error) {
                console.error(`âŒ Message Processing Error for ${msg?.key?.remoteJid}:`, error);
...
app.post('/webhook/flutterwave', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    try {
        ...
        if (!svixSecret) {
            console.error("âŒ SVIX_SECRET not set");
            return;
        }
        ...
        if (order && paidAmount >= order.amount * 0.95) {
            ...
            if (vSock) {
                await vSock.sendMessage(`${order.customerPhone}@s.whatsapp.net`, {
                    text: `âœ… *PAYMENT CONFIRMED!*\n\nYour payment of â‚¦${paidAmount.toLocaleString()} has been received. Your order for *${order.productName}* is confirmed! ðŸŽ‰`
                });

                await vSock.sendMessage(`${order.vendorPhone}@s.whatsapp.net`, {
                    text: `ðŸ’° *NEW PAID ORDER!*\n\nItem: ${order.productName}\nAmount: â‚¦${paidAmount.toLocaleString()}\nCustomer: +${order.customerPhone}`
                });
            }
        }
    } catch (e) {
...
app.post('/webhook/korapay', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    try {
        ...
        if (!signature || !secretKey) {
            console.error("âŒ Missing Korapay signature or secret key");
            return;
        }
        ...
        if (computedSignature !== signature) {
            console.error("âŒ Invalid Korapay webhook signature");
            return;
        }
...
async function bootAllVendors() {
    try {
        ...
        for (const v of vendors) {
            ...
            console.log(`ðŸ”Œ Preparing boot for ${v.storeName}...`);
            await delay(8000);
...
app.get('/', (req, res) => res.send('Naxr AI Engine Active! ðŸš€'));

const PORT = process.env.PORT || 3000;
const server = app.listen(PORT, () => console.log(`ðŸŒ Server active on port ${PORT}`));
...
io.on("connection", (socket) => {
    ...
    if (vendorPhone) {
        ...
        console.log(`ðŸ”Œ Socket client connected for vendor ${vendorPhone}`);
    }

    socket.on("register_vendor", (data) => {
        if (data?.vendor_phone) {
            socket.join(data.vendor_phone);
            console.log(`ðŸ”Œ Socket client registered for vendor ${data.vendor_phone}`);
        }
    });
...
mongoose.connect(process.env.MONGODB_URI)
    .then(() => console.log("ðŸ“¦ Connected to MongoDB Atlas Cloud!"))
...
async function gracefulShutdown(signal) {
    console.log(`âš ï¸ Received ${signal}. Commencing graceful shutdown...`);
    ...
    for (const phone in vendorSockets) {
        if (vendorSockets[phone]) {
            console.log(`ðŸ”Œ Closing connection for vendor ${phone}...`);
            try {
    ...
    if (globalSock) {
        console.log(`ðŸ”Œ Closing connection for master agent...`);
        try {
            globalSock.ev.removeAllListeners('connection.update');
            globalSock.ev.removeAllListeners('creds.update');
            globalSock.ev.removeAllListeners('messages.upsert');
    ...
    console.log("ðŸ‘‹ Shutdown complete. Exiting process.");
    process.exit(0);
}
